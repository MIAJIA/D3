...
function chartDirective(DataSource) {
    return {
		...
		link: chartlinkFunc.bind(null, DataSource)
    }
}

function chartlinkFunc(DataSource, scope, element, attrs) {

	var data;

	DataSource.get(scope.metricRange).then(function(res) {
		data = res.data[scope.metricType.value];
		draw();
	})
